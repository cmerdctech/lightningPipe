...
var moduleFunction = function(args) {
	...
	var digestQuery = function(query, path, newQuery, status) {
		...
		if (qtools.isEmpty(query)) {
			...
			var componentSchemaInfo = self.dictionary.getComponentSchemaInfo(current.value),
				fileName = current.value;

console.dir({'=-=== componentSchemaInfo =====':componentSchemaInfo});
			...
			if (typeof (componentSchemaInfo) == 'undefined' && current.value!='list') {
					...
					global.localEnvironment.log.fatal({
						...
						data: {
							err: "self.dictionary.getComponentSchemaInfo('"+current.value+"') returned no value)",
							mssqlEngineId: self.objectId,
							query: query
						}
					});
