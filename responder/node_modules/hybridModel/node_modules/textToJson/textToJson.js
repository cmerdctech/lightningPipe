...
var moduleFunction = function(args) {
	...
	var self = this,
		...
		getDefinition = function(definitionName) {
			...
			if (typeof (self.definition) == 'undefined') {
				global.localEnvironment.log.trace({
					message: 'dictionary finds no definition for ' + (definitionName.schemaName || definitionName)
				});
	...
	this.convert = function(inData, definitionName) {
		if (inData.length === 0) {
			global.localEnvironment.log.info({
				message: "Source data is empty (file or table exists but no records)",
				evidence: {schemaName:definitionName.schemaName}
			});
		...
		if (typeof (self.definition) == 'undefined') {
			return {
				status: -1,
				message: "dictionary finds no definition for " + definitionName
			}
		}
