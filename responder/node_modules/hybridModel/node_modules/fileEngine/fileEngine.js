		...
		return function(err, result, converterName) { //CALLBACK FROM ACCESSOR ============================
			...
			if (result.type == 'list') {
				...
				if (converterName){
					...
					var conversionOut = converter.convert(result.data, result.type);
					
					if (conversionOut.status==1){
						result.data=conversionOut.data;
					}
					else{
						self.fatalError=true;
						self.parentCallback({data:conversionOut});
					}
				}
			...
			if (callbacksDone()) {
				if (self.fatalError){
					return; //if we left via error, do not call callback any more. We are done here.
				}
	...
	/*
		...
		At the engine level, the responsibilities are
			...
			3)	execute the access callback that
				...
				c)	once all accesses have returned, execute the dictionary's assembler()
